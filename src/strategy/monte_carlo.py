...
def simulate_contract_probability(
    *,
    ...
    if high_so_far_f is not None:
        crossed_high = (not is_low and spec.threshold_high is not None
                        and (high_so_far_f > spec.threshold_high
                             or (high_so_far_f == spec.threshold_high and not spec.inclusive_high)))
        crossed_low = (is_low and spec.threshold_low is not None
                       and (high_so_far_f < spec.threshold_low
                            or (high_so_far_f == spec.threshold_low and not spec.inclusive_low)))
        if crossed_high or crossed_low:
            return ProbabilityEstimate(0.0, 0.0, 0.0, 0.0, simulations,
                                       forecast_error_std_f, impossible=True)
    ...
    if high_so_far_f is not None:
        sampled = (np.minimum(sampled, high_so_far_f) if is_low
                   else np.maximum(sampled, high_so_far_f))
