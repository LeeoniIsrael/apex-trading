...
def test_repeated_snapshots_count_once_and_controls_have_no_actual_profit(tmp_path):
    ...
    spec=SimpleNamespace(ticker='TEST',station_id='KAUS',official_source=SimpleNamespace(value='twc'), market_date=now.date(),measurement=SimpleNamespace(value='high'), observation_window_end=now+timedelta(hours=1),city='Austin')
